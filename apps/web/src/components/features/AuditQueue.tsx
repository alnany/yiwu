...
export function AuditQueue({ locale }: Props) {
  ...
  return (
    <div>
      ...
      {loading ? <div className="text-center py-12 text-gray-500">Loading audits...</div> :
        audits.length === 0 ? (
          ...
          <div className="space-y-3">
            {audits.map((audit) => (
              <AuditRow key={audit.id} audit={audit} locale={locale} onUpdate={updateAudit}
                expanded={expandedId === audit.id} onToggle={() => setExpandedId(expandedId === audit.id ? null : audit.id)} />
            ))}
          </div>
        )}
    </div>
    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="p-4 flex items-center gap-4 cursor-pointer" onClick={onToggle}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-400">{manufacturer?.country}</p>
        </div>
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-gray-50 pt-4">
          {manufacturer?.description && <p className="text-sm text-gray-600 mb-4">{manufacturer.description}</p>}
          {manufacturer?.tags?.length > 0 && (
            <div className="flex flex-wrap gap-1 mb-4">
              {manufacturer.tags.map((t: string) => <span key={t} className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{t}</span>)}
            </div>
          )}
          <div className="mb-4">
          ...
          {audit.status !== "completed" && (
            <div className="flex gap-3">
              <button onClick={() => onUpdate(audit.id, "fail", notes)}
              ...
              <button onClick={() => onUpdate(audit.id, "pass", notes)}
              ...
              <a href={`/${locale}/manufacturers/${audit.manufacturer_id}`}
