...
interface Props { post: Post; locale: string; onUpdate: () => void; }
...
export function PostCard({ post, locale, onUpdate }: Props) {
  ...
  async function handleLike() {
    ...
    if (res.ok) { setLiked(!liked); setLikeCount((c) => (liked ? c - 1 : c + 1)); }
  }
    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition p-5">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <div className="flex items-center gap-1">
            <a href={profileHref} className="font-semibold text-gray-900 hover:text-blue-600 text-sm">{authorName}</a>
      ...
      {post.media_urls?.length > 0 && (
        <div className="grid grid-cols-2 gap-2 mb-4">
          {post.media_urls.slice(0, 4).map((url, i) => <img key={i} src={url} alt="" className="rounded-lg w-full h-40 object-cover" />)}
        </div>
      )}
      {post.tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-4">
          {post.tags.map((tag) => <span key={tag} className="text-xs bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full">#{tag}</span>)}
        </div>
      )}
      <div className="flex items-center gap-4 pt-3 border-t border-gray-50">
        <button onClick={handleLike}
          className={`flex items-center gap-1.5 text-sm transition ${liked ? "text-red-500" : "text-gray-500 hover:text-red-400"}`}>
